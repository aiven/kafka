...
public abstract class InternalClusterResource implements ConnectResource {
    ...
    protected InternalClusterResource(AbstractConfig serverConfig) {
    ...
    public void putTaskConfigs(
        ...
        herderForRequest().putTaskConfigs(connector, taskConfigs, cb, InternalRequestSignature.fromHeaders(requestBody, headers));
    ...
    public void fenceZombies(
            ...
            final byte[] requestBody) throws Throwable {
        FutureCallback<Void> cb = new FutureCallback<>();
        herderForRequest().fenceZombieSourceTasks(connector, cb, InternalRequestSignature.fromHeaders(requestBody, headers));
        requestHandler.completeOrForwardRequest(
                cb,
                uriInfo.getPath(),
                "PUT",
