...
class DelayedRemoteFetch(remoteFetchTask: RemoteLogManager#AsyncReadTask,
  ...
  extends DelayedOperation(delayMs) {
        ...
        override def tryComplete(): Boolean = {
                delayedFetch.fetchPartitionStatus.foreach {
                        case (topicIdPartition, fetchStatus) =>
                                ...
                                try {
                                        ...
                                        case _: KafkaStorageException => // Case d
                                                debug(s"Partition $topicPartition is in an offline log directory, satisfy $delayedFetch immediately")
                                                return forceComplete()
                                        case _: UnknownTopicOrPartitionException => // Case b
                                                debug(s"Broker no longer knows of partition $topicPartition, satisfy $delayedFetch immediately")
                                                return forceComplete()
                                        case _: FencedLeaderEpochException => // Case e
                                                debug(s"Broker is the leader of partition $topicPartition, but the requested epoch " +
                                                  s"$fetchLeaderEpoch is fenced by the latest leader epoch, satisfy $delayedFetch immediately")
                                                return forceComplete()
                                        case _: NotLeaderOrFollowerException =>  // Case a
                                                debug("Broker is no longer the leader of %s, satisfy %s immediately".format(topicPartition, delayedFetch))
                                                return forceComplete()
                                }
                }
        ...
        override def onComplete():Unit = {
                ...
                delayedFetch.responseCallback(fetchPartitionData)
        }
}
