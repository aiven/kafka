    ...
    if (predicate(segmentMetadata)) {
      ...
      remoteLogMetadataManager.updateRemoteLogSegmentMetadata(
        new RemoteLogSegmentMetadataUpdate(segmentMetadata.remoteLogSegmentId(), time.milliseconds(),
          RemoteLogSegmentState.DELETE_SEGMENT_STARTED, brokerId)).thenCompose {
          ...
          if (lso < 0) {
            ...
            if (index < 0) {
              ...
              sortedSegments.slice(0, index).foreach { segment =>
                ...
                remoteLogMetadataManager.addRemoteLogSegmentMetadata(remoteLogSegmentMetadata).thenCompose(
                  new function.Function[Void, CompletableFuture[Void]] {
                    override def apply(t: Void): CompletableFuture[Void] = {
