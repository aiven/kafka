...
import kafka.log.{CleanableIndex, LocalLog, OffsetIndex, OffsetPosition, TimeIndex, TransactionIndex, TxnIndexSearchResult}
  ...
  def close(): Unit = {
    if (!closed.getAndSet(true)) {
      Array(offsetIndex, timeIndex, txnIndex).foreach(x =>
        x.renameTo(new File(CoreUtils.replaceSuffix(x.file.getPath, "", LocalLog.DeletedFileSuffix))))
    }
  }
...
class RemoteIndexCache(maxSize: Int = 1024, remoteStorageManager: RemoteStorageManager, logDir: String) extends Logging {
  ...
  private def init(): Unit = {
    ...
    Files.list(cacheDir.toPath).forEach((path: Path) => {
      if (path.endsWith(LocalLog.DeletedFileSuffix)) {
