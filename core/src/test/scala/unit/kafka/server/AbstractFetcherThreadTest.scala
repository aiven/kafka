...
class AbstractFetcherThreadTest {
  ...
  def testFollowerFetchMovedToTieredStore(): Unit = {
    ...
    fetcher.setReplicaState(partition, replicaState)
    fetcher.addPartitions(Map(partition -> initialFetchState(None, 3L, leaderEpoch = 5)))
  ...
  def testFencedOffsetResetAfterMovedToRemoteTier(): Unit = {
    ...
    fetcher.setReplicaState(partition, replicaState)
    fetcher.addPartitions(Map(partition -> initialFetchState(None, 0L, leaderEpoch = 5)))
