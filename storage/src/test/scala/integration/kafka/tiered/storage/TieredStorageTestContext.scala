...
final class TieredStorageTestContext(private val zookeeperClient: KafkaZkClient,
  ...
  def initContext(): Unit = {
    val bootstrapServerString = TestUtils.bootstrapServers(brokers, ListenerName.forSecurityProtocol(securityProtocol))
  ...
  def log(brokerId: Int, topicPartition: TopicPartition): Option[UnifiedLog] = {
