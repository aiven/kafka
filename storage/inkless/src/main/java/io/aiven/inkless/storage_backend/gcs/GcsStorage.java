...
public class GcsStorage extends StorageBackend {
     ...
     * two a resumable session needs (initiate, then upload). Above it the resumable path is kept, both
     * so that an arbitrarily large object is never buffered into a single array, and because the trade
     * inverts with size: a single request saves a fixed round trip but cannot stream, so its cost per
     * megabyte is measurably higher. Production measurement put the break-even near 2 MiB, hence a
     * threshold below it -- raising this without re-measuring makes large uploads slower, not faster.
     */
