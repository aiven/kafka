...
class TransactionsMixedVersionsTest(Test):
    ...
    def test_transactions_mixed_versions(self, old_kafka_version, metadata_quorum=quorum.isolated_kraft, group_protocol=None):
        ...
        always_or_unreachable(condition,
                              "[TransactionsMixedVersionsTest.test_transactions_mixed_versions] No duplicates",
                              {"num_dups": num_dups})
        assert condition, "Detected %d duplicates in the output stream" % num_dups

        condition = input_message_set == output_message_set
        always_or_unreachable(condition,
                              "[TransactionsMixedVersionsTest.test_transactions_mixed_versions] Input and output message sets are equal",
        ...
        always_or_unreachable(condition,
                              "[TransactionsMixedVersionsTest.test_transactions_mixed_versions] No dups in concurrently consumed messages",
        ...
        always_or_unreachable(condition,
                              "[TransactionsMixedVersionsTest.test_transactions_mixed_versions] Input and concurrently consumed output message sets are equal",
                              {"len(input_message_set)": len(input_message_set),
